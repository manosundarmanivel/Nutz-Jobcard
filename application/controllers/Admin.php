...
class Admin extends CI_Controller {
	...
	{ 
        ...
        $threeDaysBefore=$currentDate->modify('-3 day')->format('Y-m-d'); 
        ...
        *,
        ...
        ->group_start()
            ->group_start()
                ->where("remedy_1_date >=", $threeDaysBefore)
                ->where("remedy_1_date <=", $date)
            ->group_end()
            ->or_group_start()
                ->where("remedy_2_date >=", $threeDaysBefore)
                ->where("remedy_2_date <=", $date)
            ->group_end()
            ->or_group_start()
                ->where("remedy_3_date >=", $threeDaysBefore)
    ...
    public function addRemedy(){ 
        if($this->input->post()){ 
            ...
            $this->form_validation->set_rules('remedy_1', 'Remedy 1', 'required'); 
            $this->form_validation->set_rules('remedy_2', 'Remedy 2', 'required'); 
            $this->form_validation->set_rules('remedy_3', 'Remedy 3', 'required');  
            if ($this->form_validation->run() == FALSE) {
                $this->session->set_flashdata('message', array('danger', validation_errors()));
            }else{ 
                $post=$this->input->post();
                ...
                $remedy["remedy_1"]=$post["remedy_1"];
                $remedy["remedy_2"]=$post["remedy_2"];  
                $remedy["remedy_3"]=$post["remedy_3"];  
    ...
    public function viewRemedy($id){
        $data["remedy"]=$this->db->where("is_active",true)->where("customer_id",$id)->get("remedy")->row();
        $data["customer"]=$this->Help->getCustomerInfo($data["remedy"]->customer_id);
        $this->load->view("admin/sidebar");
